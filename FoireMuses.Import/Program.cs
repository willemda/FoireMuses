	...
	class Program
	{
		static void Main(string[] args)
		{
			...
			foreach (IScore score in ControllerFactory.Instance.ScoreController.GetAll())
			{
				...
				AddProperty(doc, score.Code1, "codageMelodiqueRISM");
				AddProperty(doc, score.Code2 != null ? score.Code2.Replace("-", " -") : null, "codageParIntervalles");
				...
				if (play != null)
				{
					ISource sourceBla = ControllerFactory.Instance.PlayController.GetSource(play);
					JObject textualSource = new JObject();
					AddProperty(textualSource, sourceBla.Id, "id");
					AddProperty(textualSource, score.SourceAssocieeAct, "act");
					if(score.SourceAssocieeAir!=0)
					...
					AddProperty(textualSource, play.Id, "playId");
				...
				foreach (string file in files)
				{
					...
					string nomFichier = convertirChaineSansAccent(file);
					if(nomFichier.ToLower() != "Thumbs.db".ToLower())
